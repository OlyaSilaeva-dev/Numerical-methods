...
matrix transpose (matrix &other) {
    ...
    vector<vector<double>> transposed_matrix(m, vector<double> (n, 0));
...
void Rotation_method(const matrix &_matrix, vector<double> &eigenvalues, vector<vector<double>> &eigenvectors) {
    matrix A = _matrix;
    matrix A_next;
    ...
    do {
        double max_element = EPS;
        int max_i, max_j;

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (i != j && A[i][j] > max_element) {
                    max_element = A[i][j];
                    max_i = i;
                    max_j = j;
                }
        ...
        auto fi = 1.0 / 2.0 * atan(2 * max_element / delta);

        vector<vector<double>> u_vec(n, vector<double>(n, 0));
        matrix Uk(u_vec);
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
    ...
    eigenvectors = transpose(compos).get_matrix();
}
